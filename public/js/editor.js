...
function Editor(editorUi, wrapperUi, editorCfg, data, name = "editor") {
    ...
    this.addRangeCircle = function () { // background scene_circle 
    ...
    this.load_world = async function (sceneName, frame, onFinished, noUpdateFrameInfo = true) {
        ...
        logger.log(`load ${sceneName}, ${frame}`); // logger
        ...
        if (world) {
            this.data.activate_world(
                world,
                function () {
                    self.on_load_world_finished(world, noUpdateFrameInfo);
                    if (onFinished)
                        onFinished();
