...
function psr_to_xyz_face_points(p, s, r, minGrid) {
  ...
  const world_coord = matmul(trans_matrix, points, 4)
...
function cornersAinB(boxA, boxB) {
  ...
  const cornersA = psr_to_xyz_face_points(boxAPosInB, boxA.scale, boxA.rotation, minGrid) // in world coordinates
...
function deepCopy(value, weakMap = new WeakMap()) { // 带原型链拷贝, 需要深拷贝world请使用copyWorld
...
function copyWorld(world) { // 该方法需要配合setWorld使用
  const worldData = {
    scene: world.frameInfo.scene,
    frame: world.frameInfo.frame,
    annotation: deepCopy(world.annotation.toBoxAnnotations())
  }
  return worldData
}

function setWorld(newWorld, worldData) { // 将copyWorld的数据应用至新的世界
  newWorld.frameInfo.scene = worldData.scene
  newWorld.frameInfo.frame = worldData.frame
  newWorld.annotation.reapplyAnnotation(worldData.annotation)
  return newWorld
}

function replaceWorld(oldWorld, newWorld) { // oldWorld数据应用至newWorld
  const worldData = copyWorld(oldWorld)
  newWorld.frameInfo.scene = worldData.scene
  newWorld.annotation.reapplyAnnotation(worldData.annotation)
  return newWorld
}
